...
import { EAStatistics } from "@/components/expert-advisors";
...
export default function AccountDetailPage() {
  ...
  const accountQuery = useQuery({
    ...
    refetchInterval: 1000, // Actualizar cada x segundos
  ...
  const totalHistoryItems = account?.tradesHistory?.length || 0;
  ...
  const paginatedHistory = account?.tradesHistory?.slice(startHistoryIndex, endHistoryIndex) || [];

  const totalPositionsItems = account?.positions?.length || 0;
  const totalPositionsPages = Math.ceil(totalPositionsItems / itemsPerPage);
  const startPositionsIndex = (positionsPage - 1) * itemsPerPage;
  const endPositionsIndex = startPositionsIndex + itemsPerPage;
  const paginatedPositions = account?.positions?.slice(startPositionsIndex, endPositionsIndex) || [];
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Tabs defaultValue="statistics" className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  Margen libre: ${latestSnapshot?.freeMargin.toLocaleString() || "0.00"}
                </p>
              </CardContent>
            </Card>
            ...
            <CardContent>
              {account.positions?.length === 0 ? (
                ...
                <>
                  ...
                  {/* Controles de paginación para posiciones */}
                  {totalPositionsPages > 1 && (
                    <div className="flex items-center justify-between px-2 py-4">
                      <div className="text-sm text-muted-foreground">
                        Mostrando {startPositionsIndex + 1} a {Math.min(endPositionsIndex, totalPositionsItems)} de {totalPositionsItems} posiciones
                      </div>
                      <div className="flex items-center space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setPositionsPage(prev => Math.max(prev - 1, 1))}
                        ...
                        <div className="text-sm">
                          Página {positionsPage} de {totalPositionsPages}
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setPositionsPage(prev => Math.min(prev + 1, totalPositionsPages))}
              ...
              {account.tradesHistory?.length === 0 ? (
                ...
                <>
                  <Table>
                    ...
                    <TableBody>
                      {paginatedHistory.map((trade: any) => {
                  ...
                  {totalHistoryPages > 1 && (
                    <div className="flex items-center justify-between px-2 py-4">
                      <div className="text-sm text-muted-foreground">
                        Mostrando {startHistoryIndex + 1} a {Math.min(endHistoryIndex, totalHistoryItems)} de {totalHistoryItems} operaciones
                      </div>
                      <div className="flex items-center space-x-2">
                        <Button
                          variant="outline"
        ...
        <TabsContent value="experts" className="space-y-4">
          <div>
            <h3 className="text-lg font-semibold mb-4">Estadísticas por EA</h3>
            <EAStatistics accountId={accountId} />
          </div>
        </TabsContent>
